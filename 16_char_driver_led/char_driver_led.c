...
static long hahaha_ioctl(struct file *file, unsigned int cmd, unsigned long arg){
	...
	printk(KERN_EMERG "hahaha_ioctl is success! cmd is %d,arg is %ld \n",cmd,arg);
...
int gpio_init(void)
{
	...
	for(i=0;i<LED_NUM;i++)
	{
		ret = gpio_request(led_gpio[i],"LED");
		if(ret)
		{
			printk("%d failed ,ret = %d",i,ret);
			return -1;
		}
		else
		{
			s3c_gpio_cfgpin(led_gpio[i],S3C_GPIO_OUTPUT);
